...
import rospy
from drive_toy_car import _RC_Car_Driver

STOP_THROTTLE = 10  #如果油门开度低于这个值，玩具车将停止运动
START_THROTTLE = 11.0 #这辆玩具车从静止状态启动的时候，必须给出大于这个油门开度的值，才能够启动
MAX_STEER = 100.0    #允许的最大的方向转角,单位为度
DUR_TIME = 0.3       #启动后油门的持续时间，之后，油门将自动降为停车油门值STOP_THROTTLE
ESP = 0.001
...
class XMax_Driver(_RC_Car_Driver):
    ...
    def cmd_calibration(self,throttle,steer): 
        ...
        if abs(throttle) < ESP: 
            xmax_throttle = throttle  #如果等于0，直接赋值,并将车辆状态设置为Stopped
            self.__XMax_status = Vehicle_status.Stopped
        else:
            if self.__XMax_status == Vehicle_status.Stopped:
                xmax_throttle = START_THROTTLE #以较高的速度启动车辆
                self.__XMax_status = Vehicle_status.Beginning_Moving
                self.__start_moving_time = rospy.get_time()
            elif self.__XMax_status == Vehicle_status.Beginning_Moving:
                t = rospy.get_time()
                delta_t = t - self.__start_moving_time
                if delta_t > DUR_TIME:
                    xmax_throttle = STOP_THROTTLE #车辆已经正常启动运行，改为按最低速度运行
                    self.__XMax_status = Vehicle_status.Normal_Moving
                else:
                    xmax_throttle = START_THROTTLE #继续按启动速度运行,状态保持不变
            elif self.__XMax_status == Vehicle_status.Normal_Moving:
                xmax_throttle = STOP_THROTTLE #继续按最低速度运行，状态保持不变       

        return xmax_throttle/100.0,xmax_steer/100.0
